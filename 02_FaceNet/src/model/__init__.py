...
def BatchNormalization(input=None, shape=None, name=None, epsilon=0.001):
    bn_gamma = tf.get_variable(name=name+'_w', shape=shape, trainable=False, initializer=tf.zeros_initializer())  # gamma of batch normalization
    bn_beta = tf.get_variable(name=name+'_b', shape=shape, trainable=False, initializer=tf.zeros_initializer())  # beta of batch normalization
...
def Inception_block_1a(input=None):
    ...
    inception_1a = tf.concat(values=[X_3x3, X_5x5, X_pool, X_1x1], axis=-1, name='inception_3a_concat')
    return inception_1a
...
def Inception_block_1b(input=None):
    ...
    X_pool = tf.pad(tensor=X_pool, paddings=paddings, mode='CONSTANT')
    inception_1b = tf.concat(values=[X_3x3, X_5x5, X_pool, X_1x1], axis=-1, name='inception_3b_concat')
    return inception_1b
...
def Inception_block_2a(input=None):
    ...
    X_pool = tf.pad(tensor=X_pool, paddings=paddings, mode='CONSTANT')
    inception_2a = tf.concat(values=[X_3x3, X_5x5, X_pool, X_1x1], axis=-1, name='inception_4a_concat')
    return inception_2a
...
def Inception_block_3a(input=None):
    ...
    X_pool = tf.pad(tensor=X_pool, paddings=paddings, mode='CONSTANT')
    inception_2a = tf.concat(values=[X_3x3, X_pool, X_1x1], axis=-1, name='inception_5a_concat')
    return inception_2a
...
def Inception_block_3b(input=None):
    ...
    paddings = tf.constant([[0, 0], [1, 1], [1, 1], [0, 0]])
    X_pool = tf.pad(tensor=X_pool, paddings=paddings, mode='CONSTANT')
    inception_3b = tf.concat(values=[X_3x3, X_pool, X_1x1], axis=-1, name='inception_5b_concat')
    return inception_3b
...
class FaceNet:
    ...
    def forward(self):
        ...
        # check point 3 passed: fixed the BatchNormalization confuse gamma and beta parameters
        ...
        # check point 5 passed but precision is not same

        conv2_activation_padded = tf.pad(conv2_activation, tf.constant([[0, 0], [1, 1], [1, 1], [0, 0]]), mode='CONSTANT')
        conv3_generation = Conv2D(input=conv2_activation_padded, filter_shape=[3, 3, 64, 192], strides=[1, 1, 1, 1], padding='VALID', data_format='NHWC', name='conv3')
        ...
        # check point 6 passed but precision is not same

        conv3_activation_padded = tf.pad(conv3_activation, tf.constant([[0, 0], [1, 1], [1, 1], [0, 0]]), mode='CONSTANT')
        maxpool_conv3 = tf.nn.max_pool(value=conv3_activation_padded, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID', data_format='NHWC', name='maxpool_conv3')
        # check point 1 passed but precision is not same
        inception_1a = Inception_block_1a(maxpool_conv3)
        # check point 7 passed: fixed concatenate disorder issue
